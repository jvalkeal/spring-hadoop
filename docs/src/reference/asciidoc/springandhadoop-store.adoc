...
Hadoop where the whole configuration logic works around use of an
adapter.
...
In above example we created a normal Spring `@Configuration` class
extending
...
enable some needed functionality. Then what is left for a user to do
is to override some configure methods from an adapter and use exposed
interface, namely {shdp-DataStoreTextWriterConfigurer}[DataStoreTextWriterConfigurer]
in this case to do a writer configuration. Bean of type
{shdp-DataStoreWriter}[DataStoreWriter] is then created automatically.
